...
impl HboMaxNormalizer {
    ...
    fn map_hbo_genre(&self, hbo_genre: &str) -> Vec<String> {
        match hbo_genre.to_lowercase().as_str() {
            ...
            "superhero" | "dc" | "dc universe" => vec!["Action".to_string(), "Science Fiction".to_string()],
...
mod tests {
    ...
    fn test_hbo_genre_mapping() {
        ...
        assert_eq!(normalizer.map_hbo_genre("true crime"), vec!["Crime", "Documentary"]);
        assert_eq!(normalizer.map_hbo_genre("comedy"), vec!["Comedy"]);
        assert_eq!(normalizer.map_hbo_genre("dc universe"), vec!["Action", "Science Fiction"]);
    }
