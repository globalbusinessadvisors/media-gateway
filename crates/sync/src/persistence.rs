...
impl SyncPersistence {
    ...
    pub async fn load_watchlist_state(&self, user_id: &str) -> Result<ORSet> {
        ...
        debug!("Loaded {} watchlist items for user {}", or_set.len(), user_id);
        Ok(or_set)
    }
    ...
    pub async fn load_progress_state(&self, user_id: &str) -> Result<Vec<PlaybackPosition>> {
        ...
        debug!("Loaded {} progress entries for user {}", positions.len(), user_id);
        Ok(positions)
    }
    ...
    pub async fn persist_watchlist(&self, user_id: &str, or_set: &ORSet) -> Result<()> {
        debug!("Persisting watchlist for user {} ({} items)", user_id, or_set.len());
    ...
    pub async fn persist_progress(&self, user_id: &str, position: &PlaybackPosition) -> Result<()> {
        debug!("Persisting progress for user {} content {}", user_id, position.content_id);
    ...
    pub async fn persist_device(&self, user_id: &str, device: &DeviceInfo) -> Result<()> {
        debug!("Persisting device {} for user {}", device.device_id, user_id);
    ...
    pub async fn update_device_heartbeat(&self, user_id: &str, device_id: &str) -> Result<()> {
        self.repository.update_device_heartbeat(user_id, device_id).await?;
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_persistence_manager_lifecycle() {
        ...
        if let Ok(database_url) = std::env::var("DATABASE_URL") {
            ...
            or_set.add("content-1".to_string(), HLCTimestamp::from_components(1000, 0), "device-1".to_string());

            persistence.persist_watchlist(&user_id, &or_set).await.unwrap();
