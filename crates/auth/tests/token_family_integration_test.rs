...
mod token_family_tests {
    ...
    fn create_test_managers() -> (Arc<JwtManager>, Arc<TokenFamilyManager>, Arc<SessionManager>) {
    ...
    async fn test_token_family_creation_and_tracking() {
        ...
        let family_id = token_family_manager.create_family(user_id.clone()).await.unwrap();
        ...
        // Add token to family
        token_family_manager.add_token_to_family(family_id, claims.jti.clone()).await.unwrap();

        // Verify token is in family
        let is_in_family = token_family_manager.is_token_in_family(family_id, &claims.jti).await.unwrap();
        assert!(is_in_family);
    }
    ...
    async fn test_successful_token_rotation() {
        ...
        let family_id = token_family_manager.create_family(user_id.clone()).await.unwrap();
        let token1 = jwt_manager.create_refresh_token_with_family(
            user_id.clone(),
            Some("test@example.com".to_string()),
            vec!["user".to_string()],
            vec!["read:content".to_string()],
        ...
        token_family_manager.add_token_to_family(family_id, claims1.jti.clone()).await.unwrap();

        // Simulate token rotation (normal refresh flow)
        // Step 1: Verify old token is in family
        let is_valid = token_family_manager.is_token_in_family(family_id, &claims1.jti).await.unwrap();
        assert!(is_valid);

        // Step 2: Remove old token from family
        token_family_manager.remove_token_from_family(family_id, &claims1.jti).await.unwrap();
        session_manager.revoke_token(&claims1.jti, 3600).await.unwrap();

        // Step 3: Create new token with same family
        let token2 = jwt_manager.create_refresh_token_with_family(
            user_id.clone(),
            Some("test@example.com".to_string()),
            vec!["user".to_string()],
            vec!["read:content".to_string()],
        ...
        token_family_manager.add_token_to_family(family_id, claims2.jti.clone()).await.unwrap();

        // Verify: old token NOT in family, new token IS in family
        let old_in_family = token_family_manager.is_token_in_family(family_id, &claims1.jti).await.unwrap();
        let new_in_family = token_family_manager.is_token_in_family(family_id, &claims2.jti).await.unwrap();

        assert!(!old_in_family, "Old token should be removed from family");
        assert!(new_in_family, "New token should be in family");
        assert!(session_manager.is_token_revoked(&claims1.jti).await.unwrap(), "Old token should be revoked");
    }
    ...
    async fn test_token_reuse_detection_revokes_entire_family() {
        ...
        token_family_manager.remove_token_from_family(family_id, &claims2.jti).await.unwrap();
        let token3 = jwt_manager.create_refresh_token_with_family(
            user_id.clone(),
            Some("test@example.com".to_string()),
            vec!["user".to_string()],
            vec!["read:content".to_string()],
        ...
        token_family_manager.add_token_to_family(family_id, claims3.jti.clone()).await.unwrap();

        // ATTACK: Attempt to reuse token1 (which was already rotated out)
        // This simulates a stolen token being used
        let is_token1_in_family = token_family_manager.is_token_in_family(family_id, &claims1.jti).await.unwrap();
        assert!(!is_token1_in_family, "Token1 should NOT be in family (was rotated out)");
        ...
        assert!(session_manager.is_token_revoked(&claims1.jti).await.unwrap(), "Token1 should be revoked");
        assert!(session_manager.is_token_revoked(&claims2.jti).await.unwrap(), "Token2 should be revoked");
        assert!(session_manager.is_token_revoked(&claims3.jti).await.unwrap(), "Token3 should be revoked");

        // Verify: Family no longer exists
        let family_result = token_family_manager.get_family(family_id).await;
        assert!(family_result.is_err(), "Family should be deleted after revocation");
    }
    ...
    async fn test_token_reuse_detection_with_concurrent_clients() {
        ...
        assert!(session_manager.is_token_revoked(&claims1.jti).await.unwrap());
        ...
        assert!(session_manager.is_token_revoked(&claims2.jti).await.unwrap());
    }
    ...
    async fn test_redis_performance_under_5ms() {
        ...
        let family_id = token_family_manager.create_family(user_id.clone()).await.unwrap();
        let token = jwt_manager.create_refresh_token_with_family(
            user_id.clone(),
            Some("test@example.com".to_string()),
            vec!["user".to_string()],
            vec!["read:content".to_string()],
        ...
        for _ in 0..100 {
            let _ = token_family_manager.is_token_in_family(family_id, &claims.jti).await.unwrap();
        }
        let duration = start.elapsed();
        let avg_latency = duration.as_micros() / 100;

        println!("Average latency for is_token_in_family: {}μs", avg_latency);
        assert!(avg_latency < 5000, "Average latency should be under 5ms (5000μs), got {}μs", avg_latency);
    }
    ...
    async fn test_multiple_token_rotations_in_family() {
        ...
        let mut current_token = jwt_manager.create_refresh_token_with_family(
            user_id.clone(),
            Some("test@example.com".to_string()),
            vec!["user".to_string()],
            vec!["read:content".to_string()],
        ...
        token_family_manager.add_token_to_family(family_id, current_claims.jti.clone()).await.unwrap();

        // Perform 10 rotations
        for i in 0..10 {
            // Remove old token
            token_family_manager.remove_token_from_family(family_id, &current_claims.jti).await.unwrap();

            // Create new token
            current_token = jwt_manager.create_refresh_token_with_family(
                user_id.clone(),
                Some("test@example.com".to_string()),
                vec!["user".to_string()],
                vec!["read:content".to_string()],
            ...
            let is_valid = token_family_manager.is_token_in_family(family_id, &current_claims.jti).await.unwrap();
        ...
        assert_eq!(family.active_jtis.len(), 1, "Family should have exactly 1 active token after rotations");
        assert!(family.active_jtis.contains(&current_claims.jti), "Family should contain the latest token");
    }
}
